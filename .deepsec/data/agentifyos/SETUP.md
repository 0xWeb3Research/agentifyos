# Agent setup for `agentifyos`

This is a deepsec scanning workspace. Project `agentifyos` was just registered
(target: `..`). Setup is incomplete — `data/agentifyos/INFO.md`
...
2. **Fill in `data/agentifyos/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id agentifyos
pnpm deepsec process --project-id agentifyos
```
